 ...
 * {@l #close() автоматически закроется породивший его оператор. Такое поведение
 * бывает нужно тогда, когда оператор создается в дебрях кода, а на выходе
 * получаются уже готовые данные. Например так работает метод
